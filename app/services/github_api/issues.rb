class GithubApi
  module Issues
    ...
    def move_to(board, column, number)
      issue_stat = IssueStatService.find(board, number) || IssueStatService.create!(board, column, client.issue(board.github_id, number))
      IssueStatService.move!(board, column, issue_stat)
